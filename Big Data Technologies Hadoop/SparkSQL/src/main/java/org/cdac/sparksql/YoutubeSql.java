...
public class YoutubeSql {
	...
	public static void main(String[] args) 
	{
		...
		youtube.groupBy(new Column("Category")).count().show(); //1
		youtube.select(new Column("Category")).distinct().show();  //2
		youtube.groupBy(new Column("Category")).avg("Views").show(); //3
		youtube.groupBy(new Column("Category")).max("Views").show();  //4
