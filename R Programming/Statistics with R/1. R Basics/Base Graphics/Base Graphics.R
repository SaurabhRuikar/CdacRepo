...
stars <- read.csv("Movie_Stars.csv")
#attach(stars)
...
s <- c(20,38,22,20)
pie(s,main = "Pie Chart",col = c("yellowgreen","violetred1","hotpink","dodgerblue4") , labels = i)
#legend(-3,-1,legend = i, fill = c("yellowgreen","violetred1","hotpink","dodgerblue4"),horiz = T,xpd = T)
############################################

### Histogram ########
#Sturge's Rule
k <- 1 + 3.322*log10(nrow(stars))
...
abline(v = median(stars$Income), col="red",lwd=3) # Annotating the plot
abline(h = 5, col="blue",lwd=3) # Annotating the plot


#Graphical Devices

#pdf
pdf("F:\\Statistics\\2. R Programming\\Data Exploration\\HistIncome.pdf")
...
# jpeg
jpeg("F:\\Statistics\\2. R Programming\\Data Exploration\\HistIncome.jpeg")
